...
func newGetMigrationCommand(cmdFactory Factory) *cobra.Command {
		...
		Run: func(c *cobra.Command, args []string) {
			...
			if len(clusterName) != 0 {
				...
				for _, migration := range migrations.Items {
					if migration.Spec.ClusterPair == clusterName {
	...
	getMigrationCommand.Flags().StringVarP(&clusterName, "clusterpair", "c", "", "Name of the cluster pair for which to list migrations")
...
func newDeleteMigrationCommand(cmdFactory Factory) *cobra.Command {
	...
	deleteMigrationCommand.Flags().StringVarP(&clusterName, "clusterPair", "c", "", "Name of the ClusterPair for which to delete ALL migrations")
